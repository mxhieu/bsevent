                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-block">
                            ...
                            <form action="@if(empty($ItemSupplierInfo)){{route('addscmitem')}}@else {{route('editscmitem', $ItemSupplierInfo->id)}} @endif" method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <div class="row">
                                    <div class="form-group col-md-12 @if($errors->has('supplier_id'))has_error @endif">
                                        <label for="name">Nhà Cung cấp</label>
                                        <select name="supplier_id" id="supplier_id" class="form-control init_select2" style="width:100%;">
                                            {{-- <option value="">Chọn nhà cung cấp</option> --}}
                                            @foreach($supplierList as $row)
                                            <option @if(!empty($ItemSupplierInfo) && $row->id == $ItemSupplierInfo->supplier_id) selected @elseif(old('supplier_id') == $row->id) selected @endif value="{{$row->id}}">{{$row->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
...
<script type="text/javascript">
    $(function(){
        ...
        $('#dataTable').DataTable({
            processing: true,
            serverSide: false,
            ajax: '{!! route('listscmitem') !!}',
            columns: [
                { data: 'stt', name: 'stt' },
                { data: 'supplier_link', name: 'supplier_link' },
                { data: 'code', name: 'code' },
                { data: 'name', name: 'name' },
