...
<div class="container-fluid">
    ...
    <ul class="nav nav-tabs">
        <li class="nav-item">
            <a class="nav-link" id="tab1" data-toggle="tab" href="#listtab">Danh sách đơn vị</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" id="tab2" data-toggle="tab" href="#addtab">@if(!empty($UnitInfo)) Cập nhật @else Thêm mới @endif</a>
        </li>
    </ul>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <form action="@if(!empty($UnitInfo)){{route('editunit', $UnitInfo->id)}}@else{{route('addunit')}}@endif" method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <div class="form-group @if($errors->has('name'))has_error @endif">
                                    <label for="name">Đơn vị</label>
                                    <input value="@if(!empty($UnitInfo)){{$UnitInfo->name}}@elseif(!empty(old('name'))){{old('name')}}@endif" type="text" id="name" name="name" class="form-control">
                                </div>
                                <div class="form-group @if($errors->has('remark'))has_error @endif">
                                    <label for="remark">Ghi chú</label>
                                    <input value="@if(!empty($UnitInfo)){{$UnitInfo->remark}}@elseif(!empty(old('remark'))){{old('remark')}}@endif" type="text" id="remark" name="remark" class="form-control">
                                </div>
