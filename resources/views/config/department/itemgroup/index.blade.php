...
<div class="container-fluid">
    ...
    <ul class="nav nav-tabs">
        ...
        <li class="nav-item">
            <a class="nav-link" id="tab2" data-toggle="tab" href="#addtab">@if(!empty($ItemGroupInfo)) Cập nhật @else Thêm mới @endif</a>
        </li>
    </ul>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <form action="@if(!empty($ItemGroupInfo)){{route('edititemgroup', $ItemGroupInfo->id)}} @else {{route('additemgroup', $departmentInfo->id)}}@endif" method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <div class="row">
                                    <div class="form-group col-md-8 @if($errors->has('name'))has_error @endif">
                                        <label for="name">Tên nhóm</label>
                                        <div class="input-group date">
                                            <input value="@if(!empty($ItemGroupInfo)){{$ItemGroupInfo->name}}@elseif(!empty(old('name'))){{old('name')}}@endif" type="text" id="name" name="name" class="form-control">
                                        </div>
                                    </div>
                                    <div class="form-group col-md-4 @if($errors->has('code'))has_error @endif">
                                        <label for="code">Mã</label>
                                        <div class="input-group date">
                                            <input value="@if(!empty($ItemGroupInfo)){{$ItemGroupInfo->code}}@elseif(!empty(old('code'))){{old('code')}}@endif" type="text" id="code" name="code" class="form-control">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group @if($errors->has('remark'))has_error @endif">
                                    <label for="remark">Ghi chú</label>
                                    <input value="@if(!empty($ItemGroupInfo)){{$ItemGroupInfo->remark}}@elseif(!empty(old('remark'))){{old('remark')}}@endif" type="text" id="remark" name="remark" class="form-control">
                                </div>
                                <div class="form-group">
                                    <div class="col-sm-12 padding-unset">
                                        @if(!empty($ItemGroupInfo))
...
$(function() {
    $('#dataTable').DataTable({
        processing: true,
        serverSide: false,
        ajax: '{!! route('itemgroup', $departmentInfo->id) !!}',
        columns: [
            { data: 'id', name: 'id' },
            { data: 'name', name: 'name' },
            { data: 'code', name: 'mecodember' },
            { data: 'qtyItem', name: 'qtyItem' },
