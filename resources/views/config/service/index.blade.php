...
<div class="container-fluid">
    ...
    <ul class="nav nav-tabs">
        <li class="nav-item">
            <a id="tab1" class="nav-link" data-toggle="tab" href="#listtab">Danh mục dịch vụ</a>
        </li>
        <li class="nav-item">
            <a id="tab2" class="nav-link" data-toggle="tab" href="#addtab">@if(!empty($ServiceInfo)) Cập nhật @else Thêm mới @endif</a>
        </li>
    </ul>
                ...
                <div class="col-md-9">
                    <div class="card">
                        <div class="card-block">
                            ...
                            <form action="@if(!empty($ServiceInfo)){{route('editservice', $ServiceInfo->id)}}@else{{route('addservice')}}@endif" method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <div class="row">
                                    <div class="form-group col-md-6 @if($errors->has('name'))has_error @endif">
                                        <label for="name">Tên dịch vụ</label>
                                        <input value="@if(!empty($ServiceInfo)){{$ServiceInfo->name}}@elseif(!empty(old('name'))){{old('name')}}@endif" type="text" id="name" name="name" class="form-control">
                                    </div>
                                    <div class="form-group col-md-4 @if($errors->has('Servicecategory_id'))has_error @endif">
                                        <label for="servicecategory_id">Loại hàng hóa</label>
                                        <select name="servicecategory_id" class="form-control">
                                    ...
                                    <div class="form-group col-md-2 @if($errors->has('code'))has_error @endif">
                                        ...
                                        <input value="@if(!empty($ServiceInfo)){{$ServiceInfo->code}}@elseif(!empty(old('code'))){{old('code')}}@endif" type="text" id="code" name="code" class="form-control">
                                    </div> 
                                </div>
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-2 @if($errors->has('lead_time'))has_error @endif">
                                        ...
                                        <input value="@if(!empty($ServiceInfo)){{$ServiceInfo->lead_time}}@elseif(!empty(old('lead_time'))){{old('lead_time')}}@endif" name="lead_time" type="number" min="0" class="form-control">
                                    </div>
                                </div>
                                ...
                                <div class="form-group @if($errors->has('remark'))has_error @endif">
                                    ...
                                    <textarea name="remark" class="form-control">@if(!empty($ServiceInfo)){{$ServiceInfo->remark}}@elseif(!empty(old('remark'))){{old('remark')}}@endif</textarea>
                                </div>
