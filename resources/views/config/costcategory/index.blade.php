    ...
    <div class="row page-titles">
        <div class="col-md-6 col-8 align-self-center">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">Chi phí</li>
            </ol>
        </div>
    </div>
    <ul class="nav nav-tabs">
        <li class="nav-item">
            <a class="nav-link" id="tab1" data-toggle="tab" href="#listtab">Danh sách Chi phí</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" id="tab2" data-toggle="tab" href="#addtab">@if(!empty($CostCategoryInfo)) Cập nhật @else Thêm mới @endif</a>
        </li>
    </ul>
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-block">
                            ...
                            <form action="@if(!empty($CostCategoryInfo)){{route('editcostcategory', $CostCategoryInfo->id)}}@else{{route('addcostcategory')}}@endif" method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <div class="form-group  @if($errors->has('name'))has_error @endif">
                                    <label for="name">Chi phí</label>
                                    <input value="@if(!empty($CostCategoryInfo)){{$CostCategoryInfo->name}}@elseif(!empty(old('name'))){{old('name')}}@endif" type="text" id="name" name="name" class="form-control">
                                </div>
                                <div class="form-group  @if($errors->has('remark'))has_error @endif">
                                    <label for="remark">Ghi chú</label>
                                    <input value="@if(!empty($CostCategoryInfo)){{$CostCategoryInfo->remark}}@elseif(!empty(old('remark'))){{old('remark')}}@endif" type="text" id="remark" name="remark" class="form-control">
                                </div>
                                <div class="form-group">
                                    <div class="col-sm-12 padding-unset">
                                        @if(!empty($CostCategoryInfo))
